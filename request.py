...
i = 0
while i < len(data):
    print(str(i)+".",data[i]["name"],data[i]["id"])
    i+=1
user=int(input("enter the program number:"))
print(data[user]["name"],data[user]["id"])
print()
file1=data[user]["name"]+"_ "+data[user]["id"]+".json"
link2=requests.get("http://api.merakilearn.org/courses/"+data[user]["id"]+"/exercises").json()
